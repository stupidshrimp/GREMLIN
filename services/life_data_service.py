...
# Legacy file/row import-metadata columns left over from the original Excel/CSV
# importer. GREMLIN no longer reads or writes these, and the API-first Limble sync
# has no natural value for them, so they are retained only for backward
# compatibility and must not carry NOT NULL constraints. See
# ``LifeDataService._ensure_raw_import_schema``.
...
# Canonical, API-first definitions of the two raw import tables. These are created
# only when the tables do not already exist (fresh or test databases); existing
# databases keep their own definition and are modernized in place by relaxing the
# legacy NOT NULL constraints (see ``_relax_legacy_not_null_constraints``). Legacy
# columns are kept but nullable so downstream mapping/disposition/Weibull joins and
# any historical Excel/CSV rows continue to work unchanged.
MODERN_RAW_IMPORT_TABLE_STATEMENTS = (
    """
    CREATE TABLE IF NOT EXISTS import_batch (
        ...
        -- Legacy file-import metadata: optional now that imports are API-driven.
    ...
    CREATE TABLE IF NOT EXISTS raw_cmms_record (
        ...
        updated_at TEXT,
        -- Legacy CSV/Excel row metadata: optional now that imports are API-driven.
...
# Helper indexes for the API-first lookup/reporting paths. Each entry is
# ``(index_name, table, (referenced_columns...))`` and is created only when the
# table and all of its referenced columns exist, so a pre-existing legacy table
# that lacks a modern column (e.g. ``status``) is never the cause of a failure.
MODERN_RAW_IMPORT_INDEXES = (
    ("idx_import_batch_source_system", "import_batch", ("source_system",)),
...
class LifeDataService:
    ...
    def ensure_schema(self) -> None:
        ...
        # Own and modernize the API-first raw import tables before the downstream
        # tables that reference them. This runs on its own connection (see the
        # method docstring) so it must happen outside the write transaction below.
        self._ensure_raw_import_schema()
    ...
    def _quote_ident(identifier: str) -> str:
        """Return ``identifier`` safely double-quoted for use in SQL."""

        return '"' + identifier.replace('"', '""') + '"'

    def _ensure_raw_import_schema(self) -> None:
        """Own and modernize the API-first raw import tables.

        ``import_batch`` and ``raw_cmms_record`` historically came from the legacy
        Excel/CSV importer and were never created by GREMLIN itself. As the import
        layer moves to the Limble API this method:

        * creates both tables in a modern, API-first shape on fresh databases, and
        * relaxes the obsolete ``NOT NULL`` constraints on legacy file/row metadata
          columns in existing databases so an API sync never has to fabricate values
          such as ``source_row_number``.

        Legacy columns are kept (nullable) for backward compatibility, so existing
        Excel/CSV rows and every downstream mapping/disposition/Weibull join keep
        working unchanged.

        Relaxing a column constraint in SQLite requires rebuilding the table, and
        these tables are referenced by ``mapped_cmms_record``. ``PRAGMA
        foreign_keys`` cannot be toggled inside a transaction, so the rebuild runs
        on a dedicated connection with foreign keys disabled rather than sharing
        ``ensure_schema``'s write transaction.
        """

        # A read-only precheck keeps steady-state startups from taking a write lock
        # on the shared database once the tables are already modern.
        if not self._raw_import_schema_needs_write():
            return
        ...
        try:
            conn = self.connect()
            conn.execute("PRAGMA journal_mode = DELETE")
            # Foreign keys must be disabled (outside any transaction) so the table
            # rebuild can drop a table that mapped_cmms_record references.
            conn.execute("PRAGMA foreign_keys = OFF")
            self._begin_write_transaction(conn)
            for statement in MODERN_RAW_IMPORT_TABLE_STATEMENTS:
                conn.execute(statement)
            self._relax_legacy_not_null_constraints(conn, "import_batch", LEGACY_IMPORT_BATCH_COLUMNS)
            self._relax_legacy_not_null_constraints(conn, "raw_cmms_record", LEGACY_RAW_CMMS_COLUMNS)
            # Indexes are added after any rebuild so they survive it, and only when
            # their columns exist so a minimal legacy table cannot break startup.
            for index_name, table, index_columns in MODERN_RAW_IMPORT_INDEXES:
                if not self._table_exists(conn, table):
                    continue
                existing_columns = {row[1] for row in conn.execute(f"PRAGMA table_info({self._quote_ident(table)})")}
                if not all(column in existing_columns for column in index_columns):
                    continue
                column_list = ", ".join(self._quote_ident(column) for column in index_columns)
                conn.execute(
                    f"CREATE INDEX IF NOT EXISTS {self._quote_ident(index_name)} "
                    f"ON {self._quote_ident(table)} ({column_list})"
                )
            conn.commit()
        except Exception as exc:
            if conn is not None:
    ...
    def _raw_import_schema_needs_write(self) -> bool:
        """Return True when the raw import tables still need creating/modernizing.

        Performed on a read-only connection so an already-modern database does not
        pay for a write transaction (and its busy-timeout contention on the shared
        drive) on every startup. Any error opening/reading the database falls
        through to the write path, which surfaces the real failure.
        """
        ...
        try:
            ...
            return True
        try:
            for table in ("import_batch", "raw_cmms_record"):
                if not self._table_exists(conn, table):
                    return True
            existing_indexes = {
            ...
            for index_name, table, index_columns in MODERN_RAW_IMPORT_INDEXES:
                columns = {row[1] for row in conn.execute(f"PRAGMA table_info({self._quote_ident(table)})")}
                if all(column in columns for column in index_columns) and index_name not in existing_indexes:
                    return True
            for table, legacy_columns in (
                ("import_batch", LEGACY_IMPORT_BATCH_COLUMNS),
                ("raw_cmms_record", LEGACY_RAW_CMMS_COLUMNS),
            ):
                legacy_set = {column.lower() for column in legacy_columns}
                info = conn.execute(f"PRAGMA table_info({self._quote_ident(table)})").fetchall()
                if any(row[1].lower() in legacy_set and row[3] == 1 for row in info):
                    return True
            return False
        except sqlite3.Error:
            return True
    ...
    def _relax_legacy_not_null_constraints(
        self, conn: sqlite3.Connection, table: str, legacy_columns: Iterable[str]
    ) -> bool:
        """Rebuild ``table`` so the given legacy columns drop their NOT NULL constraint.

        Returns ``True`` when a rebuild was performed. The rebuild follows SQLite's
        supported table-redefinition procedure: create a replacement table, copy
        every row (preserving primary-key/rowid values so child foreign keys stay
        valid), drop the original, rename the replacement, and recreate the
        original indexes. It is a no-op when the table is absent, when none of the
        legacy columns are present-and-NOT NULL, or when another schema object
        (trigger/view) depends on the table.
        """
        ...
        not_null_legacy = [row[1] for row in info if row[1].lower() in legacy_set and row[3] == 1]
        if not not_null_legacy:
            return False

        # Verify nothing else depends on this table before rewriting it. GREMLIN
        # owns no triggers/views over the raw import tables, so finding any here
        # means an unknown dependency exists and the safe choice is to leave the
        # schema untouched rather than risk breaking it.
        ...
        if not original_sql_row or not original_sql_row[0]:
            return False

        new_table = f"{table}__modernized"
        new_ddl = self._relax_not_null_in_ddl(original_sql_row[0], new_table, legacy_set)
        if new_ddl is None:
            return False

        columns = [row[1] for row in info]
        column_list = ", ".join(self._quote_ident(column) for column in columns)
        index_sql = [
            row[0]
        ...
        row_count_before = conn.execute(
            f"SELECT COUNT(*) FROM {self._quote_ident(table)}"
        ).fetchone()[0]

        conn.execute(f"DROP TABLE IF EXISTS {self._quote_ident(new_table)}")
        conn.execute(new_ddl)
        conn.execute(
            f"INSERT INTO {self._quote_ident(new_table)} ({column_list}) "
            f"SELECT {column_list} FROM {self._quote_ident(table)}"
        )
        conn.execute(f"DROP TABLE {self._quote_ident(table)}")
        conn.execute(
            f"ALTER TABLE {self._quote_ident(new_table)} RENAME TO {self._quote_ident(table)}"
        )
        ...
        if row_count_after != row_count_before:
            raise sqlite3.IntegrityError(
                f"row count changed while modernizing {table} "
                f"({row_count_before} -> {row_count_after}); rolling back"
            )
        return True

    def _relax_not_null_in_ddl(self, ddl: str, new_table: str, legacy_columns: set[str]) -> str | None:
        """Return a ``CREATE TABLE`` statement for ``new_table`` with NOT NULL removed.

        The original ``ddl`` (taken verbatim from ``sqlite_master``) is preserved
        exactly except that the ``NOT NULL`` constraint is dropped from the column
        definitions whose name is in ``legacy_columns``. This keeps every other
        column, default, UNIQUE/CHECK/foreign-key clause and table option intact.
        Returns ``None`` when the statement cannot be parsed or nothing changed.
        """

        open_index = self._find_top_level_paren(ddl)
        if open_index is None:
            return None
        close_index = self._matching_close_paren(ddl, open_index)
        if close_index is None:
            return None

        body = ddl[open_index + 1 : close_index]
        trailing = ddl[close_index + 1 :].strip()
        ...
        for item in self._split_sql_items(body):
            column_name = self._leading_column_name(item)
            if (
                column_name is not None
                and column_name.lower() in legacy_columns
                and not self._is_table_constraint(item)
            ):
                stripped = self._strip_not_null(item)
                if stripped != item:
                    changed = True
                rebuilt_items.append(stripped)
            else:
        ...
        statement = f"CREATE TABLE {self._quote_ident(new_table)} ({','.join(rebuilt_items)})"
        if trailing:
            statement = f"{statement} {trailing}"
        return statement

    @staticmethod
    def _find_top_level_paren(ddl: str) -> int | None:
        """Return the index of the column-list opening paren, ignoring quoted text."""

        quote: str | None = None
        for index, char in enumerate(ddl):
            if quote:
                if char == quote:
                    quote = None
                continue
    ...
    @staticmethod
    def _matching_close_paren(ddl: str, open_index: int) -> int | None:
        """Return the index of the paren that closes the one at ``open_index``."""
    ...
    def _split_sql_items(body: str) -> list[str]:
        """Split a column/constraint list on top-level commas, respecting quotes/parens."""
    ...
    @staticmethod
    def _leading_column_name(item: str) -> str | None:
        """Return the (unquoted) column name that an item defines, if any."""
    ...
    def _is_table_constraint(item: str) -> bool:
        """Return True when an item is a table-level constraint rather than a column."""

        match = re.match(r"\s*([A-Za-z]+)", item)
        if not match:
            return False
        return match.group(1).upper() in {"CONSTRAINT", "PRIMARY", "UNIQUE", "CHECK", "FOREIGN", "KEY"}

    @staticmethod
    def _strip_not_null(item: str) -> str:
        """Remove a top-level ``NOT NULL`` (and any ON CONFLICT action) from a column def.

        Only the column-level ``NOT NULL`` token is removed; ``NOT NULL`` appearing
        inside a CHECK expression (parenthesized) or a quoted literal is preserved,
        and whitespace inside quoted literals is left untouched.
        """

        pattern = re.compile(
            r"\bNOT\s+NULL(\s+ON\s+CONFLICT\s+(?:ROLLBACK|ABORT|FAIL|IGNORE|REPLACE))?",
            re.IGNORECASE,
        )
        result: list[str] = []
        depth = 0
        ...
        while index < length:
            ...
            if char == ")":
                ...
                index += 1
                continue
            if depth == 0 and char in ("N", "n"):
                match = pattern.match(item, index)
                if match:
                    # Drop the constraint, then close the gap it leaves between the
                    # surrounding tokens without disturbing any other whitespace.
                    index = match.end()
                    while index < length and item[index] in " \t":
                        index += 1
                    if index >= length:
