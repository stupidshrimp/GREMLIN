...
class LifeDataService:
    ...
    def ensure_schema(self) -> None:
        ...
        with self.write_connection() as conn:
            ...
            conn.executescript(
                """
                -- API-first raw import tables. GREMLIN owns these so a fresh
                -- database has the modern shape; legacy file/row metadata columns
                -- are kept but nullable for backward compatibility with historical
                -- Excel/CSV rows. They are created before mapped_cmms_record, which
                -- references them. Pre-existing databases keep their own definition.
                ...
                CREATE INDEX IF NOT EXISTS idx_import_batch_source_system ON import_batch(source_system);
                ...
                CREATE INDEX IF NOT EXISTS idx_raw_cmms_record_import_batch ON raw_cmms_record(import_batch_id);
                CREATE INDEX IF NOT EXISTS idx_raw_cmms_record_source ON raw_cmms_record(source_system, source_record_id);
                CREATE INDEX IF NOT EXISTS idx_raw_cmms_record_content_hash ON raw_cmms_record(raw_content_hash);
