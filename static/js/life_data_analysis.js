  ...
  function renderDispositionEditor(data) {
    ...
    const card = el("section", { class: "glass-card lda-card" }, [
      ...
      el("div", { class: "lda-row-actions" }, [
        ...
        withTooltip(
          download,
          `Downloads the entries you selected — ${
            data.scope === "new"
              ? `only the new / undispositioned ${recordWord} rows`
              : `every eligible ${recordWord} row`
          } for this asset — as an .xlsx workbook with the disposition dropdowns built in, dates and ` +
            "numbers typed so the columns sort, so you can fill them in offline."
        ),
        withTooltip(
          upload,
